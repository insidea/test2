...
class ContentOperations
{
	...
	* added by JN
	*/
	function CreateMultipleHierarchyDropdown($current = '', $parent = '', $name = 'parent_id', $allowcurrent = 0, $use_perms = 0, $ignore_current = 0, $allow_all = false, $use_name = null,$from_parent=false)
	{
		$result = '';
		$userid = -1;
		...
		if ($allcontent !== FALSE && count($allcontent) > 0)
		{
			...
			foreach ($allcontent as $one)
			{
				...
				//pokud neni parent id shodne se zadanou defaultni hodnotou
				if($from_parent!==false && $one->ParentId()!=$from_parent){
				...
				if (!$one->WantsChildren()) continue;
				{
					...
					// Select current parentS if it exists
					foreach($parent as $selec){
						if ($one->Id() == $selec)
						{
					...
					if( ($value == -1) && ($ignore_current == 0) )
					{
						$content .= '>'.$one->Hierarchy().'. - '.$txt.' ('.lang('invalid').')</option>';
					}
					else
					{
						$content .= '>'.$one->Hierarchy().'. - '.$txt.'</option>';
					}
				}
				$sorting_array[] = array('id'=>$one->Id(),'content'=>$content);
			}

		}

		/* sesortujeme podle defaultni hodnoty, to zachova pozice selectnutych prvku */
		$used_buffer = array();
		foreach($parent AS $par){
			foreach($sorting_array AS $block){
				if($block['id']==$par){
					$result .= $block['content'];
					array_push($used_buffer,$block['id']);
				}
			}
		}

		foreach($sorting_array AS $block){
			if(!in_array($block['id'],$used_buffer)){
				$result .= $block['content'];
			}
		}
		...
		if( !empty($result) ){
			...
			$result = '<select name="'.$name.'[]" id="'.$name.'" class="multiselect" size="6" multiple>'.$result.'</select>';
		}else{
			$result = '<pre><strong>Bohužel se současným nastavením tohoto pole není k dispozici žádný výsledek k zobrazení</strong><br /><div style="font-size:10px;">(pravděpodobně je pole nastaveno zobrazovat děti stránky která zatím žádné děti nemá';
			if($from_parent!==false){
				$result .= ' [parent id nastaveno na: '.$from_parent.']';
			}
